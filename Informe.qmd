...
cuantias_2023 <- cuantias_2023 |> 
  mutate(Marginal = ifelse(Cuantia_ajustada < quantile(x = Cuantia_ajustada,prob = q), T,F),
...
Sacar_muestra_ep <- function(n, Marginal, XL, mu_2){
...
Muestra_cuantia <- Sacar_muestra(n = 3000,Marginal = Marginal, XL = XL,mu_2 = mu_2)
...
prima_pura_total <- mean(cuantia_total)
cuantil_0.01 <- quantile(cuantia_total, probs = 0.99)

margen_solvencia_minimo <- as.numeric(cuantil_0.01 - prima_pura_total)
