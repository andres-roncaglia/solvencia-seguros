...
data.frame(cuantia_total = c(metodo_1, metodo_2, metodo_3),
           metodo = rep(c("1","2","3"), each = length(metodo_1))) |> 
  ggplot() +
...
data.frame(cuantia_total = c(metodo_1, metodo_2, metodo_3)/1000,
           metodo = rep(c("Wei+Par", "Lognormal", "Emp+Par", "Empírica"),
                        each = length(metodo_1), 
                        levels = c("Wei+Par", "Lognormal", "Emp+Par", "Empírica"))) |> 
  group_by(metodo) |> 
  summarise(
    min(cuantia_total),
    quantile(cuantia_total, probs = 0.01),
...
La distribución de la cuantía total simulada en base al método Weibull+Pareto parece ser el método más conservador, ya que en más del 50% de las simulaciones, la cuantía total toma un valor mayor a 1800 millones de pesos. En cambio, el método basado en la distribución empírica con una cola Pareto, es el más arriesgado, ya que poco más del 1% de los valores simulados superaron los 1800 millones. Sin embargo, el método que 
